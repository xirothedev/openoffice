...
//
// Sends a single prompt, streams events to stdout, and exits when the
// session goes idle. Also supports `--command` for slash-command execution,
// `--format json` for raw event streaming, `--continue` / `--session` for
// session resumption, and `--fork` for forking before continuing.
